import { p as useNuxtApp } from "./Bogji2Ua.js";
var define_process_env_default = {};
const adsenseConfig = {
  // 실제 애드센스 클라이언트 ID로 변경하세요
  clientId: "ca-pub-6441782123701736",
  // 테스트 모드 설정 (localhost 및 개발 환경에서 테스트 광고 표시)
  testMode: define_process_env_default.NUXT_PUBLIC_ADSENSE_TEST_MODE === "true" || window.location.hostname === "localhost",
  // 각 페이지별 광고 슬롯 ID
  slots: {
    main: "1234567890",
    footer: "0987654321",
...
function getAdsenseConfig() {
  {
    ...
    try {
      ...
      if ($configEnv) {
        clientId = $configEnv.NUXT_PUBLIC_ADSENSE_CLIENT_ID || clientId;
        testMode = $configEnv.NUXT_PUBLIC_ADSENSE_TEST_MODE === "true" || testMode;
      }
    } catch (error) {
      console.log("Config.env not available for AdSense, using defaults");
    }
    try {
